...
// double stime,etime; // computing starting/ending time    //! error: ‘stime’ redeclared as different kind of symbol
double time1,time2;
...
void init_alignment()
{
   ...
   MPI_Sendrecv(a, dl2, MPI_DOUBLE, get_index(my_row,my_col-my_row,sp), 1,
            tmp_a, dl2, MPI_DOUBLE, get_index(my_row,my_col+my_row,sp), 1, MPI_COMM_WORLD, &status);
   memcpy(a, tmp_a, dl2 * sizeof(double) );

   /*将B中坐标为(i,j)的分块B(i,j)向上循环移动j步*/
   MPI_Sendrecv(b, dl2, MPI_DOUBLE, get_index(my_row-my_col,my_col,sp), 1,
            tmp_b, dl2, MPI_DOUBLE, get_index(my_row+my_col,my_col,sp), 1, MPI_COMM_WORLD, &status);
   memcpy(b, tmp_b, dl2 * sizeof(double) );
}
...
void main_shift()
{
   ...
   for(l=0; l<sp; l++)
   {
     ...
     for(i=0; i<dl; i++)
      ...
      /* 将分块a左移1位 */
      // it would be blocked!
      //MPI_Send(a , dl2, MPI_DOUBLE, get_index(my_row, my_col-1, sp), 1, MPI_COMM_WORLD);
      //MPI_Recv(a , dl2, MPI_DOUBLE, get_index(my_row, my_col+1, sp), 1, MPI_COMM_WORLD, &status);

   MPI_Sendrecv(a, dl2, MPI_DOUBLE, get_index(my_row,my_col-1,sp), 1,
            tmp_a, dl2, MPI_DOUBLE, get_index(my_row,my_col+1,sp), 1, MPI_COMM_WORLD, &status);
   memcpy(a, tmp_a, dl2 * sizeof(double) );

 
   MPI_Sendrecv(b, dl2, MPI_DOUBLE, get_index(my_row-1,my_col,sp), 1,
            tmp_b, dl2, MPI_DOUBLE, get_index(my_row+1,my_col,sp), 1, MPI_COMM_WORLD, &status);
   memcpy(b, tmp_b, dl2 * sizeof(double) );


      /* 将分块b上移1位 */
      // blocked possible!
      //MPI_Send(b , dl2, MPI_DOUBLE, get_index(my_row-1, my_col, sp), 1, MPI_COMM_WORLD);
      //MPI_Recv(b , dl2, MPI_DOUBLE, get_index(my_row+1, my_col, sp), 1, MPI_COMM_WORLD, &status);
   }
}
