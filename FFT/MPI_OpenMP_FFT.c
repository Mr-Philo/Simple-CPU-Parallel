...
int main(int argc, char *argv[])
{
    ...
    // 指定OpenMP线程数
    int num_of_threads = atoi(argv[1]);     // OpenMP线程数
    ...
    if(rank == 0)
    {
        ...
        // 修改为自动生成一个更长的序列
        variableNum = atoi(argv[2]);     // 命令行输入一个矩阵维度
        for(i = 0; i < variableNum; i ++)
        {
            p[i].r = (double) (rand() % 10);
            p[i].i = 0.0;
            q[i].r = (double) (rand() % 10);
            q[i].i = 0.0;
        }

        if_print = atoi(argv[3]);
        if (if_print){
            printf("Randomly generate input data:\n");
            printf("p(t) = ");
            print(p, variableNum);
            printf("q(t) = ");
            print(q, variableNum);
...
// mpicc FFT_mix.c -o FFT_mix -lm -fopenmp
// mpirun -np 4 FFT_mix 2 512 0
// 参数顺序：openmp线程数、待求解序列维数、是否输出序列结果
